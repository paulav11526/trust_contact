...
class ApplyContactForce:
    def __init__(self, model, data, df, random_row, contact_pub, point_pub, node):
        ...
        self.application_time = 10 # how many timesteps force is applied for
        self.force_starttime = 1000 # timesteps when force application begins
        self.logging_start = 1000 # timestep when sensor data logging starts
        self.logging_end = 2000 # timestep when sensor data logging ends
    ...
    def apply_force(self, force_point, force_vector, timestep, viewer):
        ...
        c_msg = Point()
        c_msg.x = float(globalpoint[0])
        c_msg.y = float(globalpoint[1])
        c_msg.z = float(globalpoint[2])
...
def main(args=None):
    ...
    with mujoco.viewer.launch_passive(node.m, node.d) as viewer:
        ...
        while viewer.is_running():
            ...
            msg.effort = (node.d.qfrc_actuator ).tolist()
